void Backward(int distance) {
  ...
  double check_out = abs((en_FL + en_FR + en_BL + en_BR) / 4);
  if (check_out > tick) {
    motor_FR.stop();
    motor_BR.stop();
    motor_FL.stop();
  ...
  int left_output = (base_speed + output) * -1;
  int right_output = (base_speed - output) * -1;
